...
const Search = ({ data }) => {
  ...
  //   query: { test, subid },
  // } = router;
  var shopper_id = process.env.NEXT_PUBLIC_STORE_ID;

  const [visibleProduct, setVisibleProduct] = useState(18);
  // const { productData, setSortedField } = useState({data});
          ...
          <div className="flex w-full">
            <div className="w-full">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-6 gap-2 md:gap-3 lg:gap-3">
                {/* {productData&&productData?.slice(0, visibleProduct).map((product, i) => (
                 
                    <ProductCard key={i + 1} product={product} /> */}
                {productsbycat.data &&
                  productsbycat.data?.map((product, i) => (
                    <ProductCard key={i + 1} product={product} />
                  ))}
              </div>
...
//   products = data.filter(
//     (product) =>
//       product.sub_category_id.toLowerCase().replace('&', '').split(' ').join('-') ===
//       category
//   );
// }
