...
import { Shopwebdetails } from "@services/storeapi";
import { SetDetail } from "src/storage/redux/actions/detailActions";
          ...
          <div className="top-bar h-16 lg:h-auto flex items-center justify-between py-4 mx-auto">
            <Link href="/">
              <a className="mr-3 lg:mr-12 xl:mr-12 hidden md:hidden lg:block">
                <Image width={110} height={70} src={`${logo}`} alt="logo" />
              </a>
            </Link>
            ...
            <div className="hidden md:hidden md:items-center lg:flex xl:block absolute inset-y-0 right-0 pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
              ...
              <button
                ...
                aria-label="Login"
              >
                {imageUrl || userInfo?.data.avatar ? (
                  <Link href="/user/dashboard">
                    <a className="relative top-1 w-6 h-6">
                      <Image
                        width={29}
                        height={29}
                        src={imageUrl || userInfo?.data.avatar}
                        alt="user"
